...
frappe.provide("shoper.integrations");
	...
	refresh: function (frm) {
		if (frm.doc.enabled) {
			frm.add_custom_button(__('Link a new bank account'), () => {
				new shoper.integrations.plaidLink(frm);
			});
			...
			frm.add_custom_button(__("Sync Now"), () => {
				frappe.call({
					method: "shoper.shoperprimeho_integrations.doctype.plaid_settings.plaid_settings.enqueue_synchronization",
					freeze: true,
					callback: () => {
...
shoper.integrations.plaidLink = class plaidLink {
	constructor(parent) {
		this.frm = parent;
	...
	plaid_success(token, response) {
		...
		frappe.prompt({
			...
			frappe.xcall('shoper.shoperprimeho_integrations.doctype.plaid_settings.plaid_settings.add_institution', {
				token: token,
				response: response
			}).then((result) => {
				frappe.xcall('shoper.shoperprimeho_integrations.doctype.plaid_settings.plaid_settings.add_bank_accounts', {
					response: response,
					bank: result,
					company: me.company
				});
