...
frappe.provide("shoper.maintenance");
frappe.ui.form.on('Maintenance Visit', {
	...
	onload: function (frm) {
		...
		if (frm.doc.maintenance_type === "Scheduled") {
			...
			frappe.call({
				method: "shoper.maintenance.doctype.maintenance_schedule.maintenance_schedule.get_serial_nos_from_schedule",
				args: {
	...
	customer_address: function (frm) {
		shoper.utils.get_address_display(frm, 'customer_address', 'address_display');
	},
...
shoper.maintenance.MaintenanceVisit = class MaintenanceVisit extends frappe.ui.form.Controller {
	refresh() {
		...
		if (this.frm.doc.docstatus === 0) {
			this.frm.add_custom_button(__('Maintenance Schedule'),
				function () {
					...
					shoper.utils.map_current_doc({
						method: "shoper.maintenance.doctype.maintenance_schedule.maintenance_schedule.make_maintenance_visit",
			...
			this.frm.add_custom_button(__('Warranty Claim'),
				function () {
					shoper.utils.map_current_doc({
						method: "shoper.support.doctype.warranty_claim.warranty_claim.make_maintenance_visit",
...
extend_cscript(cur_frm.cscript, new shoper.maintenance.MaintenanceVisit({frm: cur_frm}));
