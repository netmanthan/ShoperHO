...
class TestFormLoads(FrappeTestCase):
	...
	def test_load(self):
		shoperprimeho_modules = frappe.get_all("Module Def", filters={"app_name": "shoperprimeho"}, pluck="name")
		doctypes = frappe.get_all(
			"DocType",
			{"istable": 0, "issingle": 0, "is_virtual": 0, "module": ("in", shoperprimeho_modules)},
			pluck="name",
		)
