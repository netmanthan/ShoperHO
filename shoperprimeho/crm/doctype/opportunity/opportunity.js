...
frappe.provide("shoperprimeho.crm");
...
frappe.ui.form.on("Opportunity", {
	...
	party_name: function(frm) {
		...
		if (frm.doc.opportunity_from == "Customer") {
			shoperprimeho.utils.get_party_details(frm);
		} else if (frm.doc.opportunity_from == "Lead") {
			shoperprimeho.utils.map_current_doc({
				method: "shoperprimeho.crm.doctype.lead.lead.make_opportunity",
	...
	customer_address: function(frm, cdt, cdn) {
		shoperprimeho.utils.get_address_display(frm, 'customer_address', 'address_display', false);
	},

	contact_person: shoperprimeho.utils.get_contact_details,
	...
	currency: function(frm) {
		let company_currency = shoperprimeho.get_currency(frm.doc.company);
	...
	make_supplier_quotation: function(frm) {
		frappe.model.open_mapped_doc({
			method: "shoperprimeho.crm.doctype.opportunity.opportunity.make_supplier_quotation",
			frm: frm
		})
	},
	...
	make_request_for_quotation: function(frm) {
		frappe.model.open_mapped_doc({
			method: "shoperprimeho.crm.doctype.opportunity.opportunity.make_request_for_quotation",
			frm: frm
		})
	},
...
shoperprimeho.crm.Opportunity = class Opportunity extends frappe.ui.form.Controller {
	onload() {
	...
	setup_queries() {
		...
		if (me.frm.doc.opportunity_from == "Lead") {
			me.frm.set_query('party_name', shoperprimeho.queries['lead']);
		}
		else if (me.frm.doc.opportunity_from == "Customer") {
			me.frm.set_query('party_name', shoperprimeho.queries['customer']);
	...
	create_quotation() {
		frappe.model.open_mapped_doc({
			method: "shoperprimeho.crm.doctype.opportunity.opportunity.make_quotation",
			frm: cur_frm
		})
	}
	...
	make_customer() {
		frappe.model.open_mapped_doc({
			method: "shoperprimeho.crm.doctype.opportunity.opportunity.make_customer",
			frm: cur_frm
		})
	}

	show_notes() {
		const crm_notes = new shoperprimeho.utils.CRMNotes({
			frm: this.frm,
	...
	show_activities() {
		const crm_activities = new shoperprimeho.utils.CRMActivities({
			frm: this.frm,
...
extend_cscript(cur_frm.cscript, new shoperprimeho.crm.Opportunity({frm: cur_frm}));

cur_frm.cscript.item_code = function(doc, cdt, cdn) {
	var d = locals[cdt][cdn];
	if (d.item_code) {
		return frappe.call({
			method: "shoperprimeho.crm.doctype.opportunity.opportunity.get_item_details",
