shoperprimeho.ProductSearch = class {
	constructor(opts) {
	...
	bindSearchAction() {
		...
		this.searchBox.on("input", (e) => {
			...
			frappe.call({
				method: "shoperprimeho.templates.pages.product_search.search",
				args: {
					query: query
				},
				callback: (data) => {
	...
	populateResults(product_results) {
		...
		product_results.forEach((res) => {
			let thumbnail = res.thumbnail || '/assets/shoperprimeho/images/ui-states/cart-empty-state.png';
			html += `
