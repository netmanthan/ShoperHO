frappe.pages['bom-comparison-tool'].on_page_load = function(wrapper) {
	...
	new shoperprimeho.BOMComparisonTool(page);
}

shoperprimeho.BOMComparisonTool = class BOMComparisonTool {
	constructor(page) {
		this.page = page;
		this.make_form();
	}
	...
	fetch_and_render() {
		...
		frappe.call('shoperprimeho.manufacturing.doctype.bom.bom.get_bom_diff', {
			bom1: name1,
			bom2: name2
		}).then(r => {
			let diff = r.message;
