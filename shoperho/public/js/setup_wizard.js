...
frappe.setup.on("before_load", function () {
	shoperho.setup.slides_settings.map(frappe.setup.add_slide);
});

shoperho.setup.slides_settings = [
	{
		// Organization
		name: 'organization',
		...
		set_fy_dates: function (slide) {
			...
			if (country) {
				let fy = shoperho.setup.fiscal_years[country];
		...
		load_chart_of_accounts: function (slide) {
			...
			if (country) {
				frappe.call({
					method: "shoperho.accounts.doctype.account.chart_of_accounts.chart_of_accounts.get_charts_for_country",
		...
		charts_modal: function(slide, chart_template) {
			...
			let coa_tree = new frappe.ui.Tree({
				...
				label: parent,
				expandable: true,
				method: 'shoperho.accounts.utils.get_coa',
				args: {
